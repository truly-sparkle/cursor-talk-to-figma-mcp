...
server.tool(
  ...
  {
    collectionId: import_zod.z.string().optional().describe("If provided, only variables in this collection are returned")
  },
  async ({ collectionId }) => {
    try {
...
variableTool(
  ...
  {
    name: import_zod.z.string().min(1).describe("Collection name, e.g. 'colors' or 'spacing'")
  },
...
variableTool(
  ...
  {
    collectionId: import_zod.z.string().describe("Target collection id"),
    name: import_zod.z.string().min(1).describe("Variable name, e.g. 'color/brand/500' (slashes create groups)"),
    type: import_zod.z.enum(["BOOLEAN", "FLOAT", "STRING", "COLOR"]).describe("Variable type"),
    value: import_zod.z.any().optional().describe("Optional initial value for the default mode")
  },
...
variableTool(
  ...
  {
    variableId: import_zod.z.string().describe("Variable id"),
    modeId: import_zod.z.string().describe("Mode id (from the parent collection's modes)"),
    value: import_zod.z.any().describe("Value matching the variable's type")
  },
...
variableTool(
  ...
  {
    collectionId: import_zod.z.string().describe("Collection id"),
    name: import_zod.z.string().min(1).describe("Mode name, e.g. 'dark'")
  },
...
variableTool(
  ...
  {
    collectionId: import_zod.z.string().describe("Collection id"),
    modeId: import_zod.z.string().describe("Mode id to rename"),
    name: import_zod.z.string().min(1).describe("New mode name")
  },
...
variableTool(
  ...
  {
    collectionId: import_zod.z.string().describe("Collection id"),
    modeId: import_zod.z.string().describe("Mode id to remove")
  },
  (_r) => `Removed mode`
);
